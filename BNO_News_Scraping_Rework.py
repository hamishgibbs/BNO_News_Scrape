...
soup = BeautifulSoup(r.text, 'html.parser')
#%%
test_string_1 = '5 new cases and 1 new death'
test_string_2 = '2 new cases'
test_string_3 = '3 new deaths'
       
#%%
def get_raw_text_data(main_elements):
    data = []
    for element in main_elements:
        
        if element.name in ['h4', 'p'] and 'The following is a timeline' not in element.text:
            date = element.text + ' 2020'
            date = datetime.strptime(date, '%d %B %Y')
                    
    
        #add each li record to the dat
        if element.name == 'ul':
            for item in element:
                if item.name == 'li':
                    text = item.text
...
def default_clean_strings(scraped_data):
    
    s_parsed = []
    for s in scraped_data['text']:
        ...
        s = s.replace(' (Source 2)', '')
        
        s_parsed.append(s)
    

    scraped_data['text'] = s_parsed
...
def count_cases(s):
    ...
    if 'first case' in s or 'First case' in s:
        new_case = 1

    return(int(new_case))

def count_cases_test():
    assert count_cases(test_string_1) == 5
    assert count_cases(test_string_2) == 2
    assert count_cases(test_string_3) == 0    

def count_deaths(s):
    
    new_death = 0
    
    death_split = s.split('new death')
    ...
    if 'first death' in s or 'First death' in s:
        new_death = 1
    
    return(int(new_death))    
    
def count_deaths_test():
    assert count_deaths(test_string_1) == 1
    assert count_deaths(test_string_2) == 0
    assert count_deaths(test_string_3) == 3
...
def parse_strings(data):
    ...
    for s in data['text']:
        ...
        number_of_periods = sum([c == '.' for c in s])
        
        if number_of_periods > 1:
            
            #capture cases that include previously reported deaths
            sentences = s.split('.')
            
            if 'previous' in s:
                case = count_cases(sentences[0])
                death = count_deaths(sentences[0])
                
               
        else:
            case = count_cases(s[0])
            death = count_deaths(s[0])
        ...
        parsed_data.append({'date':date, 'case':case, 'death':death})
...
scraped_data = get_raw_text_data(soup.find(id = 'mvp-content-main'))
clean_strings = default_clean_strings(scraped_data)
parse_strings(clean_strings)
#%%
...
nm = pd.DataFrame(non_matching_text)
nm = nm.drop_duplicates()

nm.to_csv('./BNO_non_matching_text.csv')    
...
sd.to_csv('./BNO_Scraped_International.csv')
